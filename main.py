...
def run_sw_experiments(v: int, e: int) -> None:
    ...
    for beta in [0.01, 0.05, 0.1]:
        ...
        plot_all_distributions(g, sw_prefix.joinpath("beta_{:.0e}".format(beta)))
...
def run_cm_experiments(v: int, e: int) -> None:
    ...
    for p in [0.5, 0.6, 0.7]:
        g = copy_model(v, p)
        plot_all_distributions(g, cm_prefix.joinpath("alpha_{:.0e}_cm".format(p)))
...
def run_lpa_experiments(v: int, e: int) -> None:
    ...
    for alpha in [-0.85, -0.5, 0.0, 5.0, 100.0]:
        ...
        plot_all_distributions(g, lpa_prefix.joinpath("alpha_{:0e}_lpa".format(alpha)))


if __name__ == "__main__":
    logging.basicConfig(filename="mainLog.log", filemode="w", level=logging.INFO)
    v = 3000
    e = 90000
